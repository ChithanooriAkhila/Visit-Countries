...
class VisitCountries extends Component {
  ...
  addToVisitedCountries = id => {
    const {countriesList} = this.state
    const changedCountriesList = countriesList.map(country =>
      country.id === id ? {...country, isVisited: true} : country,
    )

    this.setState({countriesList: changedCountriesList})
  }

  deleteFromVisitedCountries = id => {
    const {countriesList} = this.state
    const changedCountriesList = countriesList.map(country =>
    ...
    return (
      <div className="main-container">
        ...
        <VisitedCountries
          ...
          deleteFromVisitedCountries={this.deleteFromVisitedCountries}
        />
      </div>
    )
  }
